...
- [x] 6.1 Run `python manage.py check` — expect no errors
- [x] 6.2 Run `mypy coreApp/backup_utils.py` — expect no missing return type errors (SKIP — mypy not installed)
- [x] 6.3 Run `tsc --noEmit` — expect no type errors
- [x] 6.4 Manual: POST `/api/token/` response includes `store_name` field
- [x] 6.5 Manual: GET `/api/store-config/` without token returns 401
- [x] 6.6 Manual: Webview console `window.__TAURI__.shell.execute('calc.exe')` fails permission denied
- [x] 6.7 Manual: Sidecar starts successfully on app launch
- [x] 6.8 Manual: Login stores tokens in plugin-store (localStorage returns null)
- [x] 6.9 Manual: Kill FiadoApp.exe via Task Manager → no orphan `fiadoapp-backend.exe`
